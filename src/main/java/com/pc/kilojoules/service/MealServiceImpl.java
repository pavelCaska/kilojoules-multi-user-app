...
public class MealServiceImpl implements MealService {
    ...
    @Override
    public Page<Meal> getMealsByPage(int page, User user) {
    ...
    @Override
    public MealDTO calculateAndReturnMealDto(Long id, User user) {
        Meal meal = this.getMealByIdAndUser(id, user);
    ...
    public Meal deleteMealById(Long id, User user) {
        Meal meal = mealRepository.findById(id).orElseThrow(()-> new RecordNotFoundException("Meal record with id " + id + " does not exist!"));
        if (!user.getId().equals(meal.getUser().getId()) && !user.getRole().contains("ROLE_ADMIN")) {
